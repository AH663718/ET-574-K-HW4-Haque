...
class TestDiceGame(unittest.TestCase):
    ...
    def test_die_custom_sides_and_roll_bounds(self):
        # Die with 10 sides should have sides==10 and roll within 1..10
        die = Die(sides=10)
        self.assertEqual(die.sides, 10)
        for _ in range(50):
            self.assertIn(die.roll(), range(1, 11))

    def test_die_invalid_sides_raises(self):
        # Creating a die with less than 1 side should raise ValueError
        with self.assertRaises(ValueError):
            Die(sides=0)

    def test_play_round_total_is_sum_of_rolls(self):
        # Ensure the total returned by play_round equals the sum of the two rolls
        for _ in range(50):
            r1, r2, total, _ = self.game.play_round()
            self.assertEqual(total, r1 + r2)
